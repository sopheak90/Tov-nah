...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('insertLocation')}}" method="POST">
                {{ csrf_field() }}
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="recipient-name" class="col-form-label">Khmer Price: </label>
                        <input type="text" name="khmerprice" class="form-control" id="recipient-name">
                    </div>

                    <div class="mb-3">
                        <label for="recipient-name" class="col-form-label">Foriegner Price: </label>
                        <input type="text" name="foriegnprice" class="form-control" id="recipient-name">
                    </div>

                    <div class="mb-3">
                        <label for="recipient-name" class="col-form-label">Estimate: </label>
                        <input type="text" name="estimate" class="form-control" id="recipient-name">
                    </div>
            ...
            <div class="card-body">
                <div class="card-body">
                    <table id="dataTable" class="table" style="width:100%">
                        <form action="/listLocation']}}" method="POST">
                            {{ csrf_field() }}
                            {{ method_field('PUT') }}
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{$i['name']}}</td>
                                    <td>{{$i['khmerprice']}}</td>
                                    <td>{{$i['foriegnprice']}}</td>
                                    <td>{{$i['estimate']}}</td>
                                    <td>
                                        <a href={{"updateLocation/" . $i['id']}}> <button type="button" class="btn btn-success">Edit</button> </a>
...
<script>
    $(document).ready(function() {
        ...
        $('#dataTable').on('click', '.deletebtn', function() {
            ...
            $('#delete_model').attr('action', 'deleteLocation/' + data[0]);
